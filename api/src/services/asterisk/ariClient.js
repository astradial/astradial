...
class AsteriskARIClient extends EventEmitter {
  ...
  async handleAiAgentCall(channel, wssUrl, orgId, callRecord) {
    try {
      ...
      const mediaBindHost = process.env.API_RTP_BIND_HOST || '0.0.0.0';

      await new Promise((resolve, reject) => {
        udpRelay.bind(0, mediaBindHost, (err) => {
      ...
      const mediaAdvertiseHost = process.env.API_RTP_ADVERTISE_HOST || (relayAddr.address === '0.0.0.0' ? '127.0.0.1' : relayAddr.address);
      ...
      const externalChannel = await this.client.channels.externalMedia({
        ...
        external_host: `${mediaAdvertiseHost}:${relayAddr.port}`,
        format: 'ulaw',
        encapsulation: 'rtp',
        transport: 'udp'
      });
