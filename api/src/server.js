...
app.post('/api/v1/calls/click-to-call', authenticateOrg, async (req, res) => {
  try {
    ...
    // For internal extensions (3-4 digits), skip DID resolution —
    // Asterisk routes these internally via PJSIP, no trunk needed.
    const isExtension = /^\d{3,4}$/.test(to);
    let resolvedCid;
    if (isExtension) {
      resolvedCid = caller_id || to;
    } else {
      try {
...
app.post('/api/v1/calls/originate-to-ai', authenticateOrg, async (req, res) => {
  try {
    ...
    if (/^\d{3,4}$/.test(to)) {
      channel = `PJSIP/${org.context_prefix}_${to}`;
      endpoint = `PJSIP/${org.context_prefix}_${to}`;
    } else {
...
╚════════════════════════════════════════════════════════╝
  `);
      ...
      if (process.env.CAMPAIGN_SCHEDULER_ENABLED !== '0') {
        ...
        // Start Campaign Bot WebSocket server
        try {
          const { CampaignBotServer } = require('./services/campaign-bot-server');
          const botServer = new CampaignBotServer({
            port: parseInt(process.env.BOT_WS_PORT) || 8765,
            models: require('./models'),
          });
          botServer.start();
        } catch (e) {
          console.error('Failed to start campaign bot server:', e.message);
        }
      }
