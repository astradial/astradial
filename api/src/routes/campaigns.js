...
// ── Bot management ─────────────────────────────────────────────────
router.use("/bots", require("./campaign-bots"));

// ── WhatsApp template picker ────────────────────────────────────────

router.get("/whatsapp-templates", requirePermission("campaigns.read"), async (req, res) => {
  try {
    const msg91 = require("../services/msg91Service");
    const result = await msg91.listTemplates();
    if (!result.ok) {
      return res.json({ templates: [], configured: false });
    }
    ...
    if (err.message && err.message.includes("MSG91_ADMIN_AUTH_KEY")) {
      return res.json({ templates: [], configured: false });
    }
