...
# Set up Asterisk CLI wrapper for Docker-Compose environment
cat << 'EOF' > /usr/local/bin/asterisk
#!/bin/bash
GATEWAY_IP=$(ip route 2>/dev/null | grep default | awk '{print $3}')
exec ssh -i /root/.ssh/id_rsa -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null root@${GATEWAY_IP} "docker exec astradial-test-asterisk-1 asterisk $(printf '%q ' "$@")"
EOF
chmod +x /usr/local/bin/asterisk
