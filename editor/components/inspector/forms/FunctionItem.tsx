      ...
      >
        ...
        >
          <div className="p-4 space-y-4">
            ...
            {/* State Mappings Section */}
            <div className="pt-3 border-t border-neutral-200 dark:border-neutral-700">
              <div className="flex items-center justify-between mb-3">
                <div className="text-xs font-medium opacity-80">State Mappings</div>
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-6 gap-1"
                  onClick={() => {
                    const mappings = { ...(func.state_mappings ?? {}) };
                    mappings[`key_${Date.now()}`] = "";
                    onChange({ state_mappings: mappings });
                  }}
                >
                  <Plus className="h-4 w-4" /> Add Mapping
                </Button>
              </div>
              {Object.keys(func.state_mappings ?? {}).length === 0 ? (
                <div className="text-xs opacity-40 italic py-2">
                  No state mappings. Maps function args to state variables using templates.
                </div>
              ) : (
                <div className="space-y-2">
                  {Object.entries(func.state_mappings ?? {}).map(([key, value]) => (
                    <div key={key} className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          delete mappings[key];
                          onChange({
                            state_mappings: Object.keys(mappings).length > 0 ? mappings : undefined,
                          });
                        }}
                      >
            ...
            <div className="pt-3 border-t border-neutral-200 dark:border-neutral-700">
              <div className="flex items-center justify-between mb-3">
                <div className="text-xs font-medium opacity-80">Function Actions</div>
                <Button
                  variant="ghost"
                  size="sm"
                  className="h-6 gap-1"
                  onClick={() => {
                    const actions = [...(func.post_actions ?? [])];
                    actions.push({ type: "webhook" });
                    onChange({ post_actions: actions });
                  }}
                >
                  <Plus className="h-4 w-4" /> Add Action
                </Button>
              </div>
              {(func.post_actions ?? []).length === 0 ? (
                <div className="text-xs opacity-40 italic py-2">
                  No actions. Add a webhook to trigger when this function is called.
                </div>
              ) : (
                <div className="space-y-2">
                  {(func.post_actions ?? []).map((action, idx) => (
                    <ActionItem
                      key={idx}
                      action={action}
                      index={idx}
                      onUpdate={(updates) => {
                        const actions = [...(func.post_actions ?? [])];
                        actions[idx] = { ...actions[idx], ...updates } as ActionJson;
                        onChange({ post_actions: actions });
                      }}
                      onRemove={() => {
                        const actions = (func.post_actions ?? []).filter((_, i) => i !== idx);
                        onChange({ post_actions: actions.length > 0 ? actions : undefined });
                      }}
                    />
                  ))}
                </div>
