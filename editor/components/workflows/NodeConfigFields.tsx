...
function BufferedInput({
  ...
  onChange,
  ...props
}: { value: string; onChange: (val: string) => void } & Omit<
...
function FieldWithTriggerRef({
  ...
  const hasTriggerOptions =
    triggerFields.length > 0 &&
    !["timing", "method", "operator", "priority", "offset_days", "duration"].includes(configKey);
        ...
        {config.timing === "scheduled" && (
          <>
            <FieldWithTriggerRef
              ...
              label="Run at date"
              configKey="run_at_date"
              placeholder="{trigger.checkin_date}"
            />
            <FieldWithTriggerRef
              config={config}
              onChange={onChange}
              triggerFields={triggerFields}
              label="Run at time"
              configKey="run_at_time"
              placeholder="{trigger.event_time} or 10:00"
            />
            <DropdownField
              label="Offset days"
              configKey="offset_days"
              options={[
        ...
        {config.timing === "recurring" && (
          <>
            ...
            <FieldWithTriggerRef
              ...
              triggerFields={triggerFields}
              label="Until date"
              configKey="repeat_until_date"
              placeholder="{trigger.checkout_date}"
            />
            <FieldWithTriggerRef
              config={config}
              onChange={onChange}
              ...
              label="Time"
              configKey="run_at_time"
              placeholder="{trigger.event_time} or 07:00"
            />
            <DropdownField
              label="Repeat interval"
              configKey="repeat_interval"
              options={[
        ...
        {triggerType === "webhook" && (
          <>
            <div className="space-y-1">
              ...
              <Input
                value={config.expected_fields || ""}
                onChange={(e) => onChange("expected_fields", e.target.value)}
                placeholder="name, phone, checkin_date, checkout_date"
                className="h-8 text-xs"
              />
              <p className="text-[10px] text-muted-foreground">
                Comma-separated. These become available as {"{trigger.field}"} in other nodes.
              </p>
            </div>
            <FieldWithTriggerRef
              config={config}
              ...
              placeholder="Receives hotel booking data"
            />
          </>
        )}
    ...
    <div className="space-y-3">
      ...
      {nodeType === "http_request" && (
        <>
          ...
          <FieldWithTriggerRef
            ...
            label="Body"
            configKey="body"
            placeholder='{"guest": "{trigger.name}"}'
            type="textarea"
          />
        </>
      )}
          ...
          {config.whatsapp_mode === "template" ? (
                ...
                <>
                  ...
                  {msg91Numbers.length > 1 ? (
                    <div className="space-y-1">
                      ...
                      <Select
                        ...
                        onValueChange={(v) => onChange("sender_number", v)}
                      >
                        <SelectTrigger className="h-8 text-xs">
                    ...
                    <div className="space-y-1">
                      ...
                      <Input
                        value={String(
                          msg91Numbers[0].integrated_number || msg91Numbers[0].number || ""
                        )}
                        disabled
                      ...
                      <Select
                        ...
                        onValueChange={(v) => {
                          ...
                          if (vars.length > 0) {
                            onChange(
                              "template_variables",
                              JSON.stringify(vars.map((k) => ({ key: k, value: "" })))
                            );
                          }
                        }}
                      >
                  ...
                  {config.template_name && (
                    <div className="space-y-1">
                      ...
                      <Input
                        value={config.template_language || selectedLanguage?.language || ""}
                        disabled
                        className="h-8 text-xs bg-muted"
                      />
                    </div>
                  ...
                  {config.template_name && templateVars.length > 0 && (
                    <>
                      ...
                      {(() => {
                        ...
                        try {
                          vars = JSON.parse(config.template_variables || "[]");
                        } catch {
                          vars = [];
                        }
                        // Ensure all template vars are present
                        if (vars.length === 0 || vars.length !== templateVars.length) {
                          vars = templateVars.map((k) => ({
                            key: k,
                            value: vars.find((v) => v.key === k)?.value || "",
                            manual: vars.find((v) => v.key === k)?.manual,
                          }));
                        }
                        return (
                          <div className="space-y-2">
                            {vars.map((v, i) => (
                              <div key={i} className="flex gap-1.5 items-center">
                                ...
                                {triggerFields.length > 0 &&
                                !v.manual &&
                                (v.value === "" || v.value.startsWith("{trigger.")) ? (
                                  <Select
                                    value={v.value}
                                    onValueChange={(val) => {
                                      if (val === "__manual__") {
                                        const updated = [...vars];
                                        updated[i] = { ...v, value: "", manual: true };
                                        onChange("template_variables", JSON.stringify(updated));
                                      } else {
                                        const updated = [...vars];
                                        updated[i] = { ...v, value: val, manual: false };
                                        onChange("template_variables", JSON.stringify(updated));
                                      }
                                    }}
                                  >
                                  ...
                                  <div className="flex gap-1 flex-1 items-center">
                                    <BufferedInput
                                      ...
                                      onChange={(val) => {
                                        const updated = [...vars];
                                        updated[i] = { ...v, value: val, manual: true };
                                        onChange("template_variables", JSON.stringify(updated));
                                      }}
                                    ...
                                    {triggerFields.length > 0 && (
                                      <Button
                                        ...
                                        title="Switch to trigger variable"
                                        onClick={() => {
                                          const updated = [...vars];
                                          updated[i] = { ...v, value: "", manual: false };
                                          onChange("template_variables", JSON.stringify(updated));
                                        }}
                                      >
                                        {"\u21A9"}
            ...
            <>
              ...
              <FieldWithTriggerRef
                ...
                triggerFields={triggerFields}
                label="Template ID"
                configKey="template_id"
                placeholder="Optional legacy template"
              />
            </>
          )}
        </>
        ...
        <>
          ...
          {bots.length > 0 ? (
            ...
            <FieldWithTriggerRef
              ...
              label="Bot ID"
              configKey="bot_id"
              placeholder="Bot ID or leave empty"
            />
          )}
          {dids.length > 0 ? (
            <div className="space-y-1">
              ...
              <Select
                ...
                onValueChange={(v) => onChange("caller_id", v)}
              >
                <SelectTrigger className="h-8 text-xs">
                  ...
                  <div key={i} className="flex gap-1.5 items-center">
                    ...
                    {triggerFields.length > 0 ? (
                      <Select
                        ...
                        onValueChange={(val) => {
                          const updated = [...vars];
                          updated[i] = { ...v, value: val === "__manual__" ? "" : val };
                          onChange("variables", JSON.stringify(updated));
                        }}
                      >
                        <SelectTrigger className="h-7 text-xs flex-1">
      ...
      {nodeType === "create_ticket" && (
        <>
          ...
          <FieldWithTriggerRef
            ...
            triggerFields={triggerFields}
            label="Room Number"
            configKey="room_number"
            placeholder="{trigger.room}"
          />
        </>
      )}
      ...
      {nodeType === "log" && (
        <FieldWithTriggerRef
          ...
          label="Message"
          configKey="message"
          placeholder="Workflow step reached"
        />
      )}
      ...
      <Separator />
      <p className="text-[10px] text-muted-foreground">
        Use {"{trigger.field}"} to reference webhook data. Use {"{step.nodeId.field}"} for previous
        step output.
      </p>
    </div>
  );
