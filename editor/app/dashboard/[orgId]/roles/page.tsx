    ...
    <div className="flex flex-col h-full">
      ...
      <div className="px-4 sm:px-6 py-4 grid grid-cols-2 sm:grid-cols-4 gap-3">
        {ROLES.map((role, i) => (
          <div
            key={role}
            className={`rounded-lg border p-3 ${userRole === role.toLowerCase() ? "ring-2 ring-primary" : ""}`}
          >
              ...
              {PERMISSIONS.map((category) => (
                <>
                  <TableRow key={category.category} className="bg-muted/30">
                    <TableCell
                      colSpan={5}
                      className="font-semibold text-xs uppercase tracking-wide text-muted-foreground py-2"
                    >
                      {category.category}
                    </TableCell>
                  </TableRow>
