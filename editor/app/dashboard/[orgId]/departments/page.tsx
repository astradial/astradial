...
import { Plus, MoreHorizontal, Upload, Play, Pause, Trash2, Volume2, Music, MessageSquare, X, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight, ChevronUp, ChevronDown } from "lucide-react";
...
import { queues, moh, greetingsApi, tts, users as pbxUsers, type PbxQueue, type PbxUser, type QueueMember, type MohOrgClass, type MohListResponse, type Greeting, type TtsVoiceGroup, type TtsModel } from "@/lib/pbx/client";
...
export default function QueuesPage() {
  ...
  useEffect(() => {
    tts.voices().then(setVoiceGroups).catch(() => setVoiceGroups([]));
    tts.models().then((r) => setTtsModels(r.models)).catch(() => setTtsModels([]));
  }, []);
  ...
  function prettyVoice(v: string): string {
    return v.replace(/^[a-z]{2}-[A-Z]{2}-(?:Chirp3-HD-|Chirp3-|Wavenet-|Neural2-|Studio-|Standard-)/, '');
  }
  ...
  async function handleEdit() {
    ...
    try {
      ...
      await queues.update(editingQueue.id, {
        ...
        music_on_hold, greeting_id: editForm.greeting_id || null, status: editForm.status as PbxQueue["status"],
      } as any);
  ...
  const allMohForDropdown = [
    ...
    ...systemMohFiles.map((f) => ({ value: `sys:${f}`, label: f.replace(/\.wav$/, '').replace(/-/g, ' '), is_system: true, file_count: 1 })),
          ...
          <div className="overflow-auto flex-1 relative">
            ...
            <TableBody>
              {loading ? (
                ...
                <TableRow><TableCell colSpan={7} className="text-center py-8 text-muted-foreground">No queues</TableCell></TableRow>
              ) : queueList.slice((page - 1) * pageSize, page * pageSize).map((q) => (
                <TableRow key={q.id}>
                  <TableCell className="font-mono text-sm">{q.number}</TableCell>
                  <TableCell className="font-medium">{q.name}</TableCell>
                  <TableCell><Badge variant="outline" className="text-xs capitalize">{q.strategy}</Badge></TableCell>
                  <TableCell className="text-xs text-muted-foreground">
                    {allMohForDropdown.find((m) => m.value === q.music_on_hold)?.label || q.music_on_hold || "default"}
                  </TableCell>
                  <TableCell className="text-sm">{q.timeout}s</TableCell>
                  <TableCell><Badge variant={q.status === "active" ? "default" : "secondary"} className="text-xs">{q.status}</Badge></TableCell>
                  <TableCell>
                    <DropdownMenu>
                      ...
                      <DropdownMenuContent align="end">
                        ...
                        <DropdownMenuItem className="text-destructive" onClick={() => handleDelete(q.id)}>Delete</DropdownMenuItem>
                      </DropdownMenuContent>
                    </DropdownMenu>
                  </TableCell>
                </TableRow>
          ...
          {queueList.length > 10 && (
            <div className="border-t border-border/50 bg-muted/30 px-4 py-3 sticky bottom-0 z-10 flex items-center justify-between">
              <div className="hidden flex-1 text-sm text-muted-foreground lg:flex">
                Showing {(page - 1) * pageSize + 1}–{Math.min(page * pageSize, queueList.length)} of {queueList.length} entries
              </div>
              <div className="flex w-full items-center gap-8 lg:w-fit">
                ...
                <div className="ml-auto flex items-center gap-2 lg:ml-0">
                  ...
                  <Button variant="outline" className="size-8" size="icon" onClick={() => setPage(p => p + 1)} disabled={page * pageSize >= queueList.length}>
                    <span className="sr-only">Go to next page</span>
                    <ChevronRight className="size-4" />
                  </Button>
                  <Button variant="outline" className="hidden size-8 lg:flex" size="icon" onClick={() => setPage(Math.ceil(queueList.length / pageSize))} disabled={page * pageSize >= queueList.length}>
              ...
              <div className="space-y-3 py-2">
                ...
                <div className="space-y-1.5">
                  ...
                  <Select
                    ...
                    onValueChange={(v) => {
                      ...
                      const nextLang = langOk ? greetingForm.language : (def ? Object.keys(def.voicesByLanguage)[0] : greetingForm.language);
                      const nextVoice = def?.voicesByLanguage[nextLang]?.[0] || greetingForm.voice;
                      setGreetingForm({ ...greetingForm, tts_model: v, language: nextLang, voice: nextVoice });
                    }}
                  >
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent>
                ...
                <div className="grid grid-cols-2 gap-3">
                    ...
                    >
                      ...
                      <SelectContent>
                        {languagesForSelectedModel.map((lc) => {
                          ...
                          return (
                            <SelectItem key={lc} value={lc}>{fromList?.label || lc}</SelectItem>
                          );
                        })}
                      </SelectContent>
                    </Select>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <SearchableSelect
                      options={voicesForSelectedLang.map((v) => ({ value: v, label: prettyVoice(v) }))}
                ...
                {supportsStyle && (
                  <div className="space-y-1.5">
                    ...
                    <Textarea
                      ...
                      onChange={(e) => setGreetingForm({ ...greetingForm, style_instructions: e.target.value })}
                      placeholder='e.g. "Speak in a warm hotel-reception tone." or "Sound urgent and clear."'
                      maxLength={500}
                    />
                    <p className="text-xs text-muted-foreground">A short natural-language nudge for how the bot should sound. Gemini models only. Max 500 chars.</p>
                  </div>
                )}
              </div>
              <DialogFooter>
              ...
              {editingQueue?.members && editingQueue.members.length > 0 ? (
                <div className="space-y-1">
                  {(() => {
                    ...
                    // with an arrow click).
                    const sortedMembers = [...editingQueue.members].sort((a, b) =>
                      ((a.penalty ?? 0) - (b.penalty ?? 0)) || a.id.localeCompare(b.id)
                    );
                    return sortedMembers.map((m, idx) => {
                      ...
                      const applyLocal = (changes: { id: string; next: Partial<QueueMember> }[]) => {
                        if (!editingQueue) return;
                      ...
                      const swapWith = async (otherIdx: number) => {
                        ...
                        try {
                          ...
                          await queues.updateMember(editingQueue.id, lower.user_id, { penalty: newLowerPenalty });
                          await queues.updateMember(editingQueue.id, upper.user_id, { penalty: newUpperPenalty });
                          await refreshAfter();
                        } catch (e) {
                          applyLocal([
                      ...
                      return (
                      <div key={m.id} className="flex items-center gap-2 text-sm border rounded-md px-2.5 py-1.5">
                        <span className="flex-1 min-w-0 truncate flex items-center gap-1.5">
                          <span className="truncate">{m.user?.full_name || "Unknown"} <span className="text-muted-foreground">— ext {m.user?.extension}</span></span>
                        ...
                        <div className="flex items-center gap-0.5">
                          <Button
                            ...
                            disabled={!canRaise}
                            title={canRaise ? `Move up — swap with ${sortedMembers[idx - 1]?.user?.full_name || "above"}` : "Already top"}
                            onClick={() => swapWith(idx - 1)}
                          ><ChevronUp className="h-3 w-3" /></Button>
                          <span className="text-xs text-muted-foreground w-6 text-center tabular-nums">P{penalty}</span>
                          <Button
                            variant="ghost"
                            size="sm"
                            className="h-6 w-6 p-0"
                            disabled={!canLower}
                            title={canLower ? `Move down — swap with ${sortedMembers[idx + 1]?.user?.full_name || "below"}` : "Already bottom"}
                            onClick={() => swapWith(idx + 1)}
              ...
              <div className="flex items-center gap-2">
                ...
                <Button
                  ...
                  onClick={async () => {
                    ...
                    const existingPenalties = (editingQueue.members || []).map((mm) => mm.penalty ?? 0);
                    const nextPenalty = existingPenalties.length === 0
                      ? 0
                      : Math.min(10, Math.max(...existingPenalties) + 1);
                    try {
                      await queues.addMember(editingQueue.id, newMemberUserId, { ring_timeout_seconds: ringTime, penalty: nextPenalty });
