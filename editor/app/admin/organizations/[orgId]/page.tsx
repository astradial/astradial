...
import { ArrowLeft, Key, RefreshCw, Save } from "lucide-react";
...
export default function OrgDetailPage() {
  ...
  const [creds, setCreds] = useState<{ api_key: string; api_secret_plaintext: string } | null>(
    null
  );
  ...
  async function handleRotateSecret() {
    if (
      !confirm(
        "This will generate a new API secret. The old one will stop working immediately. Continue?"
      )
    )
      return;
    try {
      ...
      <div className="space-y-6">
        ...
        <section className="border rounded-lg p-4 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4 text-sm">
            ...
            <div>
              <span className="text-muted-foreground">Context Prefix:</span>{" "}
              <span className="font-mono">{org?.context_prefix as string}</span>
            </div>
            <div>
              <span className="text-muted-foreground">API Key:</span>{" "}
              <span className="font-mono text-xs">{org?.api_key as string}</span>
            </div>
          </div>
        </section>
          ...
          {creds && (
            <div className="bg-muted/30 rounded-md p-3 space-y-2 text-sm">
              <div className="flex justify-between">
                <span className="text-muted-foreground">API Key</span>
                <span className="font-mono">{creds.api_key}</span>
              </div>
              <div className="flex justify-between">
                <span className="text-muted-foreground">New Secret</span>
                <span className="font-mono text-destructive">{creds.api_secret_plaintext}</span>
              </div>
              <p className="text-xs text-muted-foreground">
                Save this now — it won&apos;t be shown again after you leave this page.
              </p>
            </div>
          )}
        </section>
              ...
              <Select value={consentMode} onValueChange={setConsentMode}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="external_consent">
                    External Consent (no in-call notice)
                  </SelectItem>
                  <SelectItem value="opt_out">
                    Opt-Out (record by default, press 2 to stop)
                  </SelectItem>
                  <SelectItem value="explicit_opt_in">
                    Explicit Opt-In (press 1 to allow recording)
                  </SelectItem>
                </SelectContent>
              </Select>
            </div>
