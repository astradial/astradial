...
export const Action = Type.Object({
  type: Type.String(), // e.g., "function", "end_conversation", "tts_say", "webhook"
  ...
  url: Type.Optional(Type.String()), // Webhook URL (for "webhook" type), supports templates like {call.channel_id}
  body: Type.Optional(Type.Any()), // Webhook request body (for "webhook" type), supports template variables
  auth: Type.Optional(Type.String()), // Auth mode: "internal" for localhost AstraPBX calls (for "webhook" type)
});
...
export const FlowFunction = Type.Object({
  ...
  state_mappings: Type.Optional(Type.Record(Type.String(), Type.String())), // Key-value pairs mapping state keys to template expressions
  post_actions: Type.Optional(Type.Array(Action)), // Actions to execute when this function is called (e.g. webhook)
});
...
// Idle timeout configuration for user silence detection
export const IdleConfig = Type.Object({
  timeout_secs: Type.Number({ default: 8 }), // Seconds of silence before prompting
  max_retries: Type.Integer({ default: 2, minimum: 1, maximum: 5 }), // Number of prompts before hanging up
  prompt_message: Type.Optional(Type.String()), // Custom prompt message (default: "Are you still there?")
  goodbye_message: Type.Optional(Type.String()), // Custom goodbye message before hanging up
});
...
export const FlowSchema = Type.Object({
  ...
  value_maps: Type.Optional(Type.Record(Type.String(), Type.Record(Type.String(), Type.String()))), // Named lookup tables e.g. { department_queues: { restaurant: "5003", reception: "5001" } }
  idle_config: Type.Optional(IdleConfig), // User silence detection and auto-hangup
...
export type IdleConfigJson = Static<typeof IdleConfig>;
export type GlobalFunctionJson = Static<typeof GlobalFunction>;
export type StateMappingsJson = Record<string, string>;
export type ValueMapsJson = Record<string, Record<string, string>>;
