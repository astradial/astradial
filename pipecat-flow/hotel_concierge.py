...
# Uses Gemini Live (native voice) across all transports
# Supports WebRTC (browser/mobile) and WebSocket (AstraPBX/Asterisk telephony)
#
...
# ─── Role Message (shared across all nodes) ───
...
route_enquiry_schema = FlowsFunctionSchema(
    ...
    properties={
        "enquiry_topic": {"type": "string", "description": "Brief description of what the guest is asking about"},
    },
...
route_raise_issue_schema = FlowsFunctionSchema(
    ...
    properties={
        "issue_summary": {"type": "string", "description": "Brief summary of the issue or request"},
    },
...
another_enquiry_schema = FlowsFunctionSchema(
    ...
    properties={
        "enquiry_topic": {"type": "string", "description": "The new enquiry topic"},
    },
    required=["enquiry_topic"],
    handler=handle_route_enquiry,
)
        ...
        task_messages=[
            {
                ...
                "content": (
                    ...
                    "2) Reservation confirmation number, "
                    "3) Number of guests, "
                    "4) Any special requests or preferences (room temperature, pillow type, "
                    "minibar preferences, allergies). "
        ...
        task_messages=[
            {
                ...
                "content": (
                    ...
                    "1) Confirm their name and room number, "
                    "2) Ask about their stay experience, "
        ...
        task_messages=[
            {
                ...
                "content": (
                    ...
                    "2) Their room number, "
                    "3) Urgency level - is this something that needs immediate attention? "
                    "4) Any preferred resolution. "
...
async def run_bot(transport: BaseTransport, runner_args: RunnerArguments):
    # Gemini Live for all transports - native voice, no separate STT/TTS
...
async def bot(runner_args: RunnerArguments):
    ...
    if isinstance(runner_args, WebSocketRunnerArguments):
        # WebSocket path: manually create transport with auto_hang_up=False
        # so it works with AstraPBX (not real Twilio, no Twilio credentials)
        ...
        transport = FastAPIWebsocketTransport(
            websocket=runner_args.websocket, params=params
        )
    else:
        # WebRTC path: use standard create_transport
